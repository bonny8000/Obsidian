---
type: map
status: active
created: 2026-05-18
updated: 2026-06-10
tags: [map, ux-research, ai, research-methods]
sources:
  - sources/measuringu-ai-usability-problem-analysis-video
  - sources/measuringu-ai-real-ui-problems-hallucinations
  - sources/sage-10778004251401851-genai-reflexive-qualitative-research
  - sources/medium-harizlim-ai-qualitative-research-2026
  - sources/brunch-ghidesigner-472
  - sources/toss-tech-research-platform-ai
  - sources/bucketplace-2026-05-06-ai-for-designers
  - sources/saeidehbakhshi-the-fallacy-of-depth-at-scale
  - sources/saeidehbakhshi-wicked-work-ai-unbundles-research
  - sources/saeidehbakhshi-long-accommodation
  - sources/tullis-albert-measuring-ux-2013
  - sources/chapman-rodden-quant-uxr-2023
  - sources/garrett-elements-ux-2011
  - sources/cooper-about-face-4-2014
  - sources/conjointly-research-methods-kb
  - sources/linkedin-user-selection-criteria
  - sources/carl-pearson-quant-uxr-self-study-resources
  - sources/quantuxblog
  - sources/meta-research-medium-publication
confidence: 0.85
---

# AI UX Research Methods

## Core Idea

This cluster tracks where AI can help UX research and where research-method integrity sets limits. The key tension is productivity versus evidence quality, interpretive responsibility, and methodological fit.

## Concepts

- [[concepts/ux-research/ux-research-automation|UX Research Automation]]
- [[concepts/ux-research/design-research-automation|Design Research Automation]]
- [[concepts/ux-research/ai-usability-analysis|AI Usability Analysis]]
- [[concepts/ux-research/ai-usability-false-alarm-triage|AI Usability False-Alarm Triage]]
- [[concepts/ux-research/huribot|Huribot]]
- [[concepts/ux-research/automated-ut-setup|Automated UT Setup]]
- [[concepts/ux-research/ai-moderated-interviews|AI-Moderated Interviews]]
- [[concepts/ux-research/reliability-vs-validity|Reliability vs Validity]]
- [[concepts/ux-research/validity-and-decision-relevance|Validity and Decision Relevance]]
- [[concepts/ux-research/any-2-agreement|Any-2 Agreement]]
- [[concepts/ux-research/evaluator-effect|Evaluator Effect]]
- [[concepts/ux-research/genai-in-qualitative-research|GenAI in Qualitative Research]]
- [[concepts/ux-research/synthetic-survey-data|Synthetic Survey Data]]
- [[concepts/ux-research/reflexive-thematic-analysis|Reflexive Thematic Analysis]]
- [[concepts/ai-agents/ai-as-thinking-partner|AI as Thinking Partner]]
- [[concepts/ux-research/methodological-integrity|Methodological Integrity]]
- [[concepts/ux-research/wicked-research-scoping|Wicked Research Scoping]]
- [[concepts/ux-research/research-ethics|Research Ethics]]
- [[concepts/ux-research/human-interpretation|Human Interpretation]]
- [[concepts/product-management/research-influence|Research Influence]]
- [[concepts/ux-research/research-operations|Research Operations]]
- [[concepts/ux-research/eight-elements-of-research-ops|Eight Elements of ResearchOps]]
- [[concepts/ux-research/rkm-model|RKM Model]]
- [[concepts/ux-research/pwdr|PWDR (People Who Do Research)]]
- [[concepts/ux-research/agentic-research-workflows|Agentic Research Workflows]]
- [[concepts/ux-research/ai-evals|AI Evals in Research]]
- [[concepts/ux-research/ai-uxr-maturity-matrix|AI UXR Maturity Matrix]]
- [[concepts/ux-research/research-methods-foundations|Research Methods Foundations]]
- [[concepts/ux-research/quant-uxr-learning-path|Quant UXR Learning Path]]
- [[concepts/ux-research/quant-uxr-rigor|Quant UXR Rigor]]
- [[concepts/ux-research/participant-selection-criteria|Participant Selection Criteria]]

## Source Path

- [[sources/measuringu-ai-usability-problem-analysis-video|MeasuringU: AI Reliability for Finding UI Problems]]
- [[sources/measuringu-ai-real-ui-problems-hallucinations|MeasuringU: Does AI Find Real UI Problems or Just Hallucinations?]]
- [[sources/sage-10778004251401851-genai-reflexive-qualitative-research|SAGE: Rejecting GenAI for Reflexive Qualitative Research]]
- [[sources/medium-harizlim-ai-qualitative-research-2026|Hariz Lim: AI in Qualitative Research (Medium, 2026)]]
- [[sources/brunch-ghidesigner-472|Brunch: Google Gemini Enterprise for UXUI Design]]
- [[sources/toss-tech-research-platform-ai|Toss Tech: Huribot Story #1]]
- [[sources/bucketplace-2026-05-06-ai-for-designers|How Designers Use AI (Bucketplace)]]
- [[sources/saeidehbakhshi-the-fallacy-of-depth-at-scale|The Fallacy of Depth at Scale (Bakhshi, 2026)]]
- [[sources/saeidehbakhshi-wicked-work-ai-unbundles-research|Saeideh Bakhshi: The Wicked Work That Surfaces When AI Unbundles Research]]
- [[sources/saeidehbakhshi-long-accommodation|Saeideh Bakhshi: The Long Accommodation]]
- [[sources/how-to-ai-uxr-2026|How To AI UXR: The ResearchOps Review (2026)]]
- [[sources/conjointly-research-methods-kb|Conjointly Research Methods Knowledge Base]]
- [[sources/linkedin-user-selection-criteria|LinkedIn: Is This User Really Our User?]]
- [[sources/carl-pearson-quant-uxr-self-study-resources|Carl Pearson: Learn Quantitative UX Research]]
- [[sources/quantuxblog|Quantitative UX Research Blog]]
- [[sources/meta-research-medium-publication|Meta Research Medium Publication]]

## Foundational UX Frameworks & Methods

These are the bedrock frameworks that AI-assisted research sits on top of. Understanding them is prerequisite for evaluating where AI augmentation is safe vs. risky.

### Frameworks & Process Models
- [[concepts/ux-research/research-methods-foundations|Research Methods Foundations]] *(Trochim / Conjointly KB)*
- [[concepts/ux-research/wicked-research-scoping|Wicked Research Scoping]] *(method framing before execution)*
- [[concepts/ux-research/validity-and-decision-relevance|Validity and Decision Relevance]] *(claim support plus decision fit)*
- [[concepts/ux-research/participant-selection-criteria|Participant Selection Criteria]] *(inclusion, exclusion, diversity checks)*
- [[concepts/ux-research/five-planes-of-ux|Five Planes of UX]] *(Garrett 2011)*
- [[concepts/ux-research/goal-directed-design|Goal-Directed Design]] *(Cooper 2014)*
- [[concepts/ux-research/heart-framework|HEART Framework]] *(Google / Chapman & Rodden)*
- [[concepts/ux-research/ux-research-matrix|UX Research Matrix]]

### UX Metrics & Measurement
- [[concepts/ux-research/self-reported-ux-metrics|Self-Reported UX Metrics]]
- [[concepts/ux-research/ux-performance-benchmarking|UX Performance Benchmarking]]
- [[concepts/ux-research/behavioral-sequence-analysis|Behavioral Sequence Analysis]]
- [[concepts/ux-research/physiological-ux-research|Physiological UX Research]]

### Quant UXR Methods & Role
- [[concepts/ux-research/maxdiff-prioritization|MaxDiff Prioritization]] *(Chapman & Rodden 2023)*
- [[concepts/ux-research/quant-uxr-role-identity|Quant UXR Role Identity]]
- [[concepts/ux-research/quant-uxr-learning-path|Quant UXR Learning Path]]
- [[concepts/ux-research/quant-uxr-rigor|Quant UXR Rigor]]
- [[concepts/ux-research/senior-uxr-career-paths|Senior UXR Career Paths]]

## Working Interpretation

AI can help with research operations and candidate issue discovery, but UX teams need explicit checks for reliability, validity, ethics, and method fit. Reflexive qualitative methods are a strong warning case: some research value comes from human interpretation itself.

Two concrete production examples from Korean product companies illustrate the "check tool" pattern: **Huribot** (Toss) reduces lightweight UT time from ~1 hour to seconds via image-based AI analysis; **Athena MCP** (Bucketplace) reduces UT setup time from 2 hours to 15 minutes via real-data prototype population. Both explicitly position AI as a supplement to, not replacement for, formal user research.

The SAGE vs. Lim tension maps a key fork in the field: SAGE argues AI is methodologically incongruent with Big Q reflexive approaches; Lim argues the problem is AI-as-analyst, not AI-as-thinking-partner. See [[concepts/ai-agents/ai-as-thinking-partner|AI as Thinking Partner]] and [[concepts/ux-research/genai-in-qualitative-research|GenAI in Qualitative Research]] for the conflict in full.

Bakhshi's "long accommodation" adds a profession-level quality gate: AI makes research generation cheaper, so the scarce work becomes proving that claims are valid and relevant to the decision being made.

The 2026 MeasuringU hallucination follow-up makes the AI review gate more concrete: AI-only usability problems should be sorted into genuine findings, false alarms, and hallucinations before they are treated as evidence. The quant UXR sources add a complementary learning path: build the basics of surveys, statistics, programming, and research design, then define rigor by decision usefulness and data quality rather than method complexity.

