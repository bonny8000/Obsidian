...
- **Sources added (12).** Agent constraint: [[wiki/sources/naver-d2-ai-hackathon-nstake|NAVER D2 NStake]] (0.87, deep, `coverage: full`), [[wiki/sources/socar-parking-brain-knowledge-graph|SOCAR parking-brain]] (0.83, deep), [[wiki/sources/polar-orbit-llm-safe-design-system|Polar Orbit]] (0.78, deep), [[wiki/sources/b2b-admin-web-accessibility|rami_ B2B accessibility]] (0.76), [[wiki/sources/membership-inference-attack-explainer|MIA explainer]] (0.74, `coverage: full`). UXR: [[wiki/sources/nngroup-accelerating-research-with-ai|NN/g research with AI]] (0.88, deep), [[wiki/sources/when-research-gets-faster|Venkat]] (0.68), [[wiki/sources/uxr-market-research-data-science-reorg|Papas reorg]] (0.60). Interfaces: [[wiki/sources/veronikapj-whats-new-android-2026|Android 2026]] (0.70, deep), [[wiki/sources/google-natively-adaptive-interfaces|Google NAI]] (0.45, **partial / `llm_ready: false`**). Reference: [[wiki/sources/pm-market-analysis-saas-vs-platform|SaaS vs. Platform]] (0.62, light), [[wiki/sources/datarize-ai-product-search|Datarize AI search]] (0.40, **vendor marketing / `llm_ready: false`**).
- **Concepts added (11):** [[wiki/concepts/ai-agents/permission-boundary-guardrails|Permission-Boundary Guardrails]], [[wiki/concepts/ai-agents/rule-statistical-external-validation|Rule/Statistical/eXternal Validation]], [[wiki/concepts/ai-agents/workflow-completeness|Workflow Completeness]], [[wiki/concepts/ai-agents/membership-inference-attack|Membership Inference Attack]], [[wiki/concepts/infrastructure-dev/domain-knowledge-graph|Domain Knowledge Graph]], [[wiki/concepts/infrastructure-dev/llm-safe-design-system|LLM-Safe Design System]], [[wiki/concepts/ux-research/web-accessibility-pour|Web Accessibility (POUR)]], [[wiki/concepts/ux-research/interpretation-locality|Interpretation Locality]], [[wiki/concepts/agent-experience/agent-invocable-app-functions|Agent-Invocable App Functions]], [[wiki/concepts/agent-experience/natively-adaptive-interfaces|Natively Adaptive Interfaces]] (draft), [[wiki/concepts/product-management/saas-vs-platform|SaaS vs. Platform]].
- **Existing concepts enriched, not duplicated (13):** `organizational-ontology` (+conflict block, 0.72 → 0.76), `deterministic-ai-workflows` (0.80 → 0.84), `ai-native-design-system` (+foreclosure claim), `ai-readable-documentation` (0.78 → 0.80), `approval-gate` (0.82 → 0.85, +2 sources), `agentic-search`, `role-convergence` (0.75 → 0.77), `uxr-role-split` (0.70 → 0.68, +contradiction), `decision-contract` (+independent corroboration), `research-influence` (`draft → active`), `ai-analysis` (0.95 → 0.90, bounds added), `ai-moderated-interviews` (0.90 → 0.92), `feature-vs-platform` (+disambiguation note).
- **Comparison added:** [[wiki/comparisons/delegate-vs-determinize|Delegate to a Model vs. Determinize in Code]] — closes the deferred action from the 2026-07-24 memo; 15 rows, each carrying its evidence grade, plus an explicit "where the table is weakest" section.
- **Analyses added (2):** [[wiki/analyses/2026-07-28-constraining-ai-by-construction|Constraining AI by Construction]] and [[wiki/analyses/2026-07-28-uxr-under-ai-acceleration|What AI Acceleration Actually Changes About UX Research]]. The 2026-07-24 memo now carries a forward-pointing partial-supersession block and its action list is marked up (1 done, 1 partial, 1 new).
- **Conflicts recorded, not merged (2):** (a) Papas's merger thesis vs. Newton's role-split thesis — same premise, opposite prediction, both pages updated; (b) SOCAR's rejection of LLM concept extraction vs. the pre-ontology meeting-records pipeline in `organizational-ontology`.
- **Capture notes:** `d2.naver.com` and `m.blog.naver.com` block normal fetching. The D2 article was retrieved through its content API and the Naver blog directly, both read end-to-end and marked `coverage: full`; the remaining ten are AI-generated extractions and are labelled as such on each page.
- **Backfill queued:** [[wiki/sources/google-natively-adaptive-interfaces|Google NAI]] sub-guides (Overview → MVP → Terminology → establish a date), a primary source for [[wiki/concepts/ai-agents/membership-inference-attack|MIA]], and Papas part 3 for the shared-state mechanism.
- **Batch reliability note:** only the carried-forward SOCAR production post measures a benefit; the five new agent-cluster sources report design rationale or a single prevented failure. Every constraint architecture in the cluster charges standing maintenance and none states its break-even volume. Approval-gate fatigue remains unmeasured and is now more load-bearing. Two sources are from SOCAR.
- **Counts:** raw 311 → 323 · sources 177 → 189 (`llm_ready` 154 → 164) · concepts 333 → 344 · comparisons 4 → 5 · analyses 4 → 6. `README.md` cluster table also corrected — it was stale from the 2026-07-27 ingest.

## 2026-07-30 — Trust measurement, AI advertising, designer-builder (3 sources)

- **Sources added (3):** [[wiki/sources/measuringu-measuring-the-ux-of-ai|Sauro & Lewis: Measuring the UX of AI]] (standard, 0.72), [[wiki/sources/kakao-vc-ai-agent-advertising|Kakao Ventures: AI agents as true representatives]] (deep, 0.76), [[wiki/sources/designer-builder-collapse|Ozenc & Yousefi: The Designer-Builder]] (standard, 0.62).
- **Concepts added (5):** [[wiki/concepts/agent-experience/delegation-spectrum|Delegation Spectrum]], [[wiki/concepts/agent-experience/principal-agent-problem|Principal–Agent Problem (AI)]], [[wiki/concepts/product-management/ai-advertising|AI Advertising]], [[wiki/concepts/ux-research/ai-ux-measurement-constructs|AI UX Measurement Constructs]] (`draft` — unvalidated instrument), [[wiki/concepts/agent-experience/designing-generative-systems|Designing Generative Systems]].
- **Existing concepts enriched (8):** `trust-calibration` (+adversarial-calibration callout, +2 sources), `agent-transparency` (0.75 → 0.72, +disclosure-underperformance warning), `checkbox-transparency` (extended from *optional* disclosure to *pushed* disclosure; now an empirical finding), `companion-attachment-dependency` (0.70 → 0.72, dependency generalized beyond companion products), `product-taste` (+"what taste is doing work for" — audience convention rather than aesthetics), `vibe-design` (0.68 → 0.70, +prototyping-kit practice and the NStake counter-case), `ux-metrics` (`draft → active`), `role-convergence` (+design discipline).
- **Analysis memo (Rule 11):** [[wiki/analyses/2026-07-30-trust-measurement-and-monetization|Measuring Trust While Trust Is Being Monetized]] — five-source evidence base (3 new + 2 existing). Core finding: **the same construct is a UX defect and a revenue asset**, so calibration guidance built for noise does not hold against an optimizing counterparty.
- **Cross-cutting revision recorded:** disclosure has now underperformed in **two independent results** — the cited Princeton figures (61.2% → 55.5% under explicit labels *plus* warnings) and Chan's willful-blindness finding. Mechanism named: disclosure works by inviting comparison, which higher delegation tiers remove. Three concept pages updated to reflect this rather than leaving the transparency-as-solution assumption in place.
- **Contradiction recorded, not merged:** [[wiki/sources/designer-builder-collapse|Yousefi]] treats fast generated prototypes as the win; [[wiki/sources/naver-d2-ai-hackathon-nstake|NStake]] observed generated UI that was wrong for its audience, with design becoming the bottleneck. Reconciliation recorded on both the new concept and `vibe-design`: generation converged, audience judgment did not.
- **Independent addition beyond the sources:** the `ai-ux-measurement-constructs` page flags that *AI Anxiety* and *Early Adoption* measure the **respondent** rather than the product and plausibly belong as covariates — a confound the anchor article does not address.
- **Batch reliability note:** the decisive Princeton figures are cited second-hand with no sample, task design, or venue; the measurement instrument has no validation; two of the memo's five sources have incentives aligned with their conclusions. All recorded on the relevant pages and in the memo's Risks section.
- **Counts:** raw 323 → 326 · sources 189 → 192 (`llm_ready` 164 → 167) · concepts 344 → 349 · analyses 6 → 7.
- **⚠ Date discrepancy flagged for a decision:** the git host clock runs ~2 days behind the session clock. Today is **2026-07-30** (independently confirmed — one source in this batch is published 2026-07-29), so the 2026-07-28 batch's `captured:` dates and its two analysis filenames are probably 2 days early. **Not corrected**: renaming linked analysis files plus rewriting ~60 already-merged pages is exactly the standalone bulk rewrite AGENTS.md Naming warns against. Needs Bonny's call.

## 2026-07-31 — Constraint architectures + reporting and modality (5 sources)

- **Sources added (5):** [[wiki/sources/karrot-kraft-design-system-agent|Karrot: Kraft]] (deep, `coverage: full`, 0.80), [[wiki/sources/pxd-writone-ai-writing-assistant|pxd: Writone]] (standard, 0.65), [[wiki/sources/maily-product-makers-guardrails|Product Makers Note #24: Guardrails]] (standard, 0.68), [[wiki/sources/carl-pearson-minimally-technical-reporting|Pearson: Minimally Technical Reporting]] (standard, 0.70), [[wiki/sources/smashing-matching-ai-modality-user-intent|Yocco: Matching AI Modality to User Intent]] (standard, 0.58).
- **Concepts added (6):** [[wiki/concepts/ai-agents/design-spec-intermediate-representation|Design Spec as Intermediate Representation]], [[wiki/concepts/ai-agents/generated-output-scoring|Generated-Output Scoring]], [[wiki/concepts/ai-agents/layered-content-guardrails|Layered Content Guardrails]], [[wiki/concepts/infrastructure-dev/in-workflow-ai-placement|In-Workflow AI Placement]], [[wiki/concepts/agent-experience/modality-intent-matching|Modality–Intent Matching]], [[wiki/concepts/ux-research/minimally-technical-reporting|Minimally Technical Reporting]].
- **Duplicate avoided (Rule 6):** a proposed `design-decision-automation` page was dropped as a near-duplicate of the existing `design-automation` and `ai-native-design-system`; Karrot's decision-not-pixels framing was added to both instead.
- **Existing concepts enriched (12):** `llm-safe-design-system` (0.76 → 0.79), `ai-native-design-system`, `harness-engineering` (0.72 → 0.78), `multi-agent-architecture`, `agent-memory`, `skill-system`, `approval-gate`, `agent-transparency`, `planning-harness`, `agent-defense-in-depth`, `designing-generative-systems` (0.62 → 0.66), `permission-boundary-guardrails`.
- **Analysis memo (Rule 11):** [[wiki/analyses/2026-07-31-constraint-architectures-converge|Constraint Architectures Converge]] — six-source evidence base (3 new + 3 existing). Core finding: the convergence on *constrain the acceptance criteria, not the generator* now spans five domains and five organisations, **and one source out of eight has ever measured the result**.
- **Comparison added:** [[wiki/comparisons/where-to-put-the-constraint|Where to Put the Constraint]] — five enforcement layers (prompt / schema / capability removal / type system + CI / post-hoc scoring) scored on bindingness, failure timing, expressiveness cost, standing cost, and self-explanation. Explicitly analytic, not empirical: no source tests two layers against each other.
- **Negative result recorded, not smoothed over:** Karrot tried a multi-agent design (separate design / coding / conductor agents collaborating freely) and abandoned it for communication overhead and context loss. Recorded on `multi-agent-architecture` and reconciled with the existing CooperBench tension — the abandoned pattern was free negotiation, which that page already identifies as the weak one. Flagged as qualitative and single-team.
- **Two three-layer models kept separate:** `agent-defense-in-depth` layers action/behavior/context for *reliability*; the new `layered-content-guardrails` layers prompt/tuning/API for *content safety*. They agree that prompts are the weakest layer and are cross-linked rather than merged.
- **Disclosure evidence updated in both directions:** Writone is recorded on `agent-transparency` as the **favourable** case — corrections cite a specific guideline page, which is checkable in a way a label is not, and fits the comparison mechanism the 2026-07-30 memo identified. Its limit is also recorded: no acceptance rate measured, and no adversarial party present.
- **Batch reliability note:** four of six memo sources are first-party accounts by the teams that built the thing; five of six are Korean industry sources; only SOCAR has ever reported outcome numbers for a constraint architecture. The measurement gap flagged on 2026-07-28 is now three sources older and unchanged.
- **Counts:** raw 326 → 331 · sources 192 → 197 (`llm_ready` 167 → 172) · concepts 349 → 355 · comparisons 5 → 6 · analyses 7 → 8. `README.md` count and cluster tables both verified against the tree.
- **Lint:** broken links 8 → 7 (one genuine fix: `ux-benchmarking` → `benchmark-studies`); no new breakage from this ingest; 0 orphans, 0 empty, 0 missing frontmatter.
