...
## Evidence
- [[concepts/ai-agents/product-evalsProduct Evals]] ??"PMs do not always need hundreds of evals; a small set of strong evals can be useful. Evals are most valuable when a feature needs clearer behavioral definition. Features such as memory and agentic task completion benefit from explicit evals."
- [[concepts/infrastructure-dev/llm-wikiLLM Wiki]] ??The three-layer architecture defines what a correct ingest output looks like: immutable raw, accurate wiki, schema compliance. Each of the five evals maps to one of these requirements.
...
- [[concepts/ai-agents/model-harnessModel Harness]] ??"Harnesses should change as model capability changes." These evals also serve as the test suite for knowing when to relax scaffolding.
