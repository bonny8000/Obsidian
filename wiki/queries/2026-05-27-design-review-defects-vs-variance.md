...
## Evidence
- [[concepts/infrastructure-dev/design-review-automation|Design Review Automation]] ??"Automated review should augment, not replace, designer judgment. Review rules need design-system and product-context awareness." Design-system rules are the defect boundary.
- [[concepts/infrastructure-dev/design-system-implementation|Design System Implementation]] ??"Design-system implementation must verify rendered output, not only code-level configuration. Typography tokens need platform and language-specific validation."
- [[concepts/infrastructure-dev/design-to-code-workflow|Design-to-Code Workflow]] ??"The goal is a loop between design, implementation, and validation rather than a one-way handoff." The validation step needs clear defect criteria to be useful.
